...
elseif ($rec == 'register_post') {
    ...
    $dou->dou_msg($_LANG['user_insert_success'], $_URL['login']);
}
...
//        $doubox .= '</div></div>';
        $doubox .= '<div id="douBox"><div class="boxBg"></div><div class="boxFrame">';
        $doubox .= '<h2><a href="javascript:void(0)" class="close" onclick="douRemove('."'douBox'".')">X</a>Tips</h2>';
        $doubox .= '<div class="boxCon"><dt>Are you sure delete？</dt><dd></dd><dd><a href="' . $_URL['order_cancel'] . '&order_sn=' . $order_sn . '">YES</a><a href="javascript:void(0)" onclick="douRemove('."'douBox'".')">NO</a></dd></div>';
        $doubox .= '</div></div>';
        echo $doubox;
    } else {
        // 取消订单
